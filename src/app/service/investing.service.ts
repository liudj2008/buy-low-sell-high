...
export class InvestingService {
  ...
  getStockPrice(stock: string): Observable<any> {
    return this._http.get(`${API_BASE_URL}${SUBDOMAIN.GET}${stock}`);
  }

  buyStock(stock: string, principle: number, price: number, date: string): Observable<any> {
  ...
  sellStock(stock: string, price: number, date: string): Observable<any> {
